...
# Xcode runs build scripts with a minimal PATH. Inherit the full PATH
# from the user's login shell so node/npm are found regardless of how
# they were installed (Homebrew, nvm, fnm, volta, mise, asdf, etc.).
...
# Build Vue app if dist is missing
